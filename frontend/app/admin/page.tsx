    ...
    >
      <div className="flex items-start justify-between">
        <div className="space-y-2">
          <p className="hidden sm:block text-[11px] font-bold uppercase tracking-widest text-stone-500">
            {label}
          </p>
...
function QuickAction({
  ...
  const sharedClassName =
    "group block bg-[#0F0F0F] border border-stone-900 rounded-xl p-4 sm:p-5 hover:shadow-md hover:shadow-stone-950/50 transition-all hover:border-amber-400/50 active:scale-[0.98] hover:-translate-y-px duration-200 ease-out";
        ...
        >
          <div className="flex items-center gap-3.5">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="hidden sm:block text-xs text-stone-400 mt-1 leading-relaxed">
                {description}
              </p>
            </div>
          </div>
...
export default function AdminDashboardPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const [totalRes, verifiedRes, contributorsRes, recentRes] =
          await Promise.all([
            // Total samples count
            supabase
              .from("lipy_samples")
            ...
            supabase
              .from("lipy_samples")
              .select(
                "contributor_id, contributor_name, session_id, created_at",
              )
  ...
  return (
    <AdminShell title="Dashboard" subtitle="Administrator overview">
      <div
        ref={containerRef}
        className="p-3 sm:p-4 md:p-5 space-y-4 w-full max-w-none flex flex-col flex-1 sm:h-full overflow-x-hidden overflow-y-visible sm:overflow-hidden"
      >
        {/* Welcome Banner */}
        <motion.div
          initial={{ opacity: 0, y: 12 }}
        ...
        >
          ...
          <div className="lg:col-span-2 flex h-full overflow-hidden">
            <div className="bg-[#0F0F0F] border border-stone-900 rounded-xl p-4 shadow-sm flex flex-col h-full w-full overflow-hidden">
              <div className="flex items-center justify-between mb-3 shrink-0">
                <div>
                  <h3 className="text-xs font-semibold uppercase tracking-[0.24em] text-stone-500 select-none font-display">
                    Recent Activity
                  </h3>
                </div>
                <Link
                  ...
                  <table className="w-full text-left border-collapse">
                    ...
                    <tbody className="divide-y divide-stone-900/30 text-xs">
                      {recentSessions.map((session) => (
                        <tr
                          key={`${session.contributor_name}-${session.session_id}-${session.created_at}`}
          ...
          <div className="flex flex-col h-full overflow-hidden">
            ...
            <div className="bg-[#0F0F0F] border border-stone-900 rounded-xl p-4 shadow-sm space-y-3 flex flex-col h-full overflow-hidden">
              <h3 className="text-xs font-semibold uppercase tracking-[0.24em] text-stone-500 font-display shrink-0">
                Quick Operations
              </h3>
              <div className="space-y-2.5 flex-1 overflow-auto pr-0.5">
                ...
                <div className="sm:hidden">
                  <QuickAction
                    title="OCR Workspace"
                    description="Recognise handwritten Odia characters"
                    icon={ScanLine}
                    href="/"
