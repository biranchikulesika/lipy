...
export function DrawCanvas({ onPredict, disabled = false, onClear, helperText = null, helperVisible = true }: DrawCanvasProps) {
	...
	return (
		<div className="flex h-full min-h-0 flex-col items-center justify-center gap-2 px-1 py-1 sm:px-2">
			...
			<div className="flex w-full max-w-[336px] items-center gap-3 mt-4">
				<button type="button" className="secondary-button flex-1 px-3 py-2 text-sm" onClick={clearCanvas}>
					Clear
				</button>
				<button type="button" className="primary-button flex-1 px-4 py-2 text-sm" onClick={handlePredict} disabled={disabled}>
					Predict
				</button>
			</div>

			{statusMessage !== "Ready" ? (
				<p className="text-[11px] uppercase tracking-[0.22em] text-slate-500 dark:text-slate-400">{statusMessage}</p>
			) : null}
		</div>
	);
}