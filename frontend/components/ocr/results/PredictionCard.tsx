...
import {
  getOdiaCharacter,
  ODIA_CHARACTER_BY_LABEL,
} from "@/lib/odiaCharacters";
...
function getConfidenceColors(confidence: number | undefined | null) {
  if (confidence == null)
    return { bar: "bg-verdigris-200", text: "text-white" };
  // Aligned with API thresholds: LOW_CONFIDENCE_THRESHOLD = 0.60, AMBIGUOUS_MARGIN = 0.10
  if (confidence >= 0.85)
    return { bar: "bg-verdigris-400", text: "text-emerald-400" };
  if (confidence >= 0.6)
    return { bar: "bg-verdigris-400", text: "text-amber-400" };
...
function getStatusDisplay(prediction: PredictionResponse) {
  ...
  if (status === "ambiguous") {
    return {
      ...
      message:
        "The model is uncertain between multiple characters. Review the top matches below.",
      color: "text-amber-300",
      iconBg: "bg-amber-400/10",
    };
  }
...
export function PredictionCard({
  ...
  const predictedCharacter =
    prediction?.character ||
    (predictedLabel ? getOdiaCharacter(predictedLabel) : null);
    ...
    <section className="relative flex min-h-0 flex-1 flex-col justify-start gap-3 pt-2 sm:pt-4 pb-2">
      ...
      <div className="relative flex flex-col justify-center min-h-55 sm:min-h-65 rounded-xl border border-white/10 bg-white/5 p-4 sm:p-5">
        ...
        {contentKey === "status" && (
          <div className="flex flex-col items-center justify-center gap-3 sm:gap-4 w-full">
            <div
              className={`relative flex h-24 w-24 shrink-0 items-center justify-center rounded-3xl ${statusDisplay?.iconBg} ${statusDisplay?.color} sm:h-28 sm:w-28`}
            >
              {statusDisplay?.icon}
            </div>
            <div className="flex-1 text-sm text-center max-w-65">
              <span
                className={`font-bold block mb-1 text-base ${statusDisplay?.color}`}
              >
                {statusDisplay?.title}
              </span>
      ...
      <div className="rounded-xl border border-white/10 bg-white/5 p-4 sm:p-5">
        ...
        <div className="flex flex-col">
          {(hasResponse
            ...
            : [
                {
                  label: "Most likely character",
                  confidence: 0,
                  character: "",
                },
                { label: "Close matching", confidence: 0, character: "" },
                {
                  label: "Another close matching",
                  confidence: 0,
                  character: "",
                },
              ]
            ...
            return (
              <div
                ...
                className="flex items-center justify-between gap-3 px-1 py-3 border-b border-white/5 last:border-0"
              >
                <div className="flex items-center gap-3">
                  <span className="flex h-6 w-6 items-center justify-center rounded-full bg-verdigris-200/10 text-[11px] font-bold text-verdigris-200">
                    {index + 1}
                  </span>
                  {hasResponse ? (
                ...
                <p
                  className={`font-mono text-sm font-bold ${hasResponse ? getConfidenceColors(item.confidence).text : "text-slate-500"}`}
                >
