      ...
      {flatNavItems.map((item) => {
        ...
        const inner = (
          <Link
            key={item.href}
            href={item.href}
            className={`group relative flex items-center gap-3 font-semibold text-sm transition-all duration-200 ${paddingClass} ${
              isActive
                ? 'border-l-2 border-l-amber-500 bg-stone-100/90 dark:bg-stone-900/60 text-stone-900 dark:text-stone-100 shadow-sm'
                : 'border border-transparent text-stone-600 dark:text-stone-400 hover:bg-stone-100/70 dark:hover:bg-stone-900/40 hover:text-stone-900 dark:hover:text-stone-200'
            }`}
          >
            <Icon className={`w-4 h-4 shrink-0 transition-transform duration-200 ${
              isActive ? 'text-amber-500 dark:text-amber-400 scale-110 translate-x-0.5' : 'group-hover:scale-110'
            }`} />
            {(!collapsed || isMobile) && (
              <span className={`transition-transform duration-200 ${isActive ? 'translate-x-1 font-bold text-stone-900 dark:text-stone-100' : 'group-hover:translate-x-0.5'}`}>
                {item.label}
              </span>
            )}
            {(!collapsed || isMobile) && isActive && (
              <ChevronRight className="w-3.5 h-3.5 ml-auto text-stone-500 dark:text-stone-400 opacity-60" />
            )}
          </Link>
        );
  ...
  return (
    <SidebarContext.Provider value={{ collapsed }}>
      <div className="h-screen bg-stone-50 dark:bg-[#070707] dark:text-[#F5F5F5] text-[#262626] font-sans selection:bg-blue-200 dark:selection:bg-blue-900 flex flex-col overflow-hidden">
        ...
        {/* Mobile Top Bar */}
        <header className="sticky top-0 z-50 h-14 border-b border-stone-200/60 dark:border-stone-900 bg-white/90 dark:bg-[#0A0A0A]/95 backdrop-blur-xl md:hidden">
          <div className="flex h-full items-center justify-between px-4 sm:px-6">
            ...
            <button
              ...
              className={`relative z-[80] flex h-10 w-10 items-center justify-center rounded-xl transition-all active:scale-95 ${mobileOpen ? 'bg-stone-100 text-stone-900 dark:bg-white/10 dark:text-white ring-1 ring-stone-200/50 dark:ring-white/10' : 'text-stone-500 hover:bg-stone-100/70 hover:text-stone-900 dark:text-stone-400 dark:hover:bg-white/5 dark:hover:text-white'}`}
            >
              <motion.div
                initial={false}
        ...
        <div className="flex flex-1 min-h-0 overflow-hidden">
          ...
          <aside
            ...
            className={`${sidebarWidth} border-r border-stone-200/60 dark:border-stone-900 bg-white dark:bg-[#0A0A0A] hidden md:flex flex-col shrink-0 transition-all duration-300 ease-in-out select-none cursor-pointer`}
          >
            {/* Sidebar Logo Area */}
            <div className="relative flex flex-col gap-3 px-3 pt-3 border-b border-stone-200/40 dark:border-stone-900/40 shrink-0">
              ...
              {(() => {
                const inner = (
                  <button
                    ...
                    className={`flex items-center gap-3 w-full rounded-xl text-red-500 hover:text-red-655 dark:text-red-400 dark:hover:text-red-300 hover:bg-red-50/50 dark:hover:bg-red-950/10 font-semibold text-sm transition-all duration-200 active:scale-95 ease-out ${
            ...
            {mobileOpen && (
              <>
                ...
                <motion.div
                  ...
                  className="fixed right-3 top-[3.9rem] z-50 w-[15rem] max-w-[calc(100vw-1.5rem)] rounded-xl border border-stone-200 bg-white/95 p-2 shadow-2xl backdrop-blur-xl dark:border-stone-900 dark:bg-[#0A0A0A]/95 md:hidden"
                >
                  <nav className="flex flex-col gap-1" aria-label="Mobile navigation">
                    {NAV_ITEMS.map((group) => (
                      <div key={group.section} className="space-y-1">
                        <div className="px-2.5 py-1.5 pt-2 text-[10px] font-bold uppercase tracking-widest text-stone-400/80 dark:text-stone-600/85 select-none">
                          {group.section}
                        </div>
                        ...
                        {group.items.map((item) => {
                          ...
                          return (
                            <Link
                              key={item.href}
                              href={item.href}
                              className={`relative flex items-center gap-2.5 text-[13px] font-semibold transition-all ${
                                isActive
                                  ? '-ml-2 pl-4 pr-2.5 py-2 rounded-l-none rounded-r-lg border-l-2 border-l-amber-500 bg-stone-100/90 dark:bg-stone-900/60 text-stone-900 dark:text-stone-100'
                                  : 'rounded-lg px-2.5 py-2 text-stone-600 hover:bg-stone-100/70 hover:text-stone-900 dark:text-stone-400 dark:hover:bg-stone-900/40 dark:hover:text-stone-200'
                              }`}
                              onClick={() => setMobileOpen(false)}
                            >
                              <Icon className={`w-3.5 h-3.5 shrink-0 transition-transform ${isActive ? 'text-amber-500 dark:text-amber-400 scale-110 translate-x-0.5' : ''}`} />
                    ...
                    <div className="my-1.5 h-px w-full bg-stone-200/80 dark:bg-stone-900/80" />

                    <button
                      onClick={handleSignOut}
                      className="flex items-center gap-2.5 rounded-lg px-2.5 py-2 text-[13px] font-semibold text-red-500 transition-colors hover:bg-red-50/70 hover:text-red-600 dark:text-red-400 dark:hover:bg-red-950/20 dark:hover:text-red-300"
                    >
